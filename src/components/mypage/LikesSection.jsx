...
import useLikedPlaces from '../../libs/hooks/useLikedPlaces';

export default function LikesSection({ userInfo, navigate }) {
  // * 전체 장소 목록을 불러오기 위한 useGetPlaces 훅
  const { getPlaces } = useGetPlaces();

  // * 좋아요 한 장소 목록을 불러오기 위한 useLikedPlaces 훅
  const { LikedPlaces } = useLikedPlaces();

  // * 내가 좋아요 한 장소 목록 필터링
  const myLikesList = LikedPlaces?.filter((place) => place.user_id === userInfo.id);

  // * 좋아요 한 장소 목록과 전체 장소 목록을 비교하여 필터링
  const filteredPlaces = getPlaces?.filter((place) =>
    myLikesList?.find((likedPlace) => likedPlace.place_id === place.id)
  );
  ...
  return (
    <div className="w-full px-4 md:px-0">
      <h2 className="text-2xl md:text-3xl font-extrabold text-gray-800 mb-6 md:mb-8 flex items-center gap-1 md:gap-2">
        ...
        좋아요 한 장소
        <span className="text-blue-500/90 text-xl md:text-2xl">({filteredPlaces?.length})</span>
      </h2>
      {/* 스크롤 컨테이너 */}
